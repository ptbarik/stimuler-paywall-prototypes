...
import { Faq, Sheet, Social, Table, TierToggle } from './Sections'
import { CTA_LABEL, PAGE_H, PLANS, TRUST_LINE } from './design'
...
export default function App() {
  ...
  const [faqExtra, setFaqExtra] = useState(0) // how much taller than collapsed the FAQ is
   ...
   * how the FAQ copy happens to wrap at a given font size. Getting this wrong
   * is not cosmetic: the CTA bar, the home indicator and the reassurance line
   * are all pinned to the page's bottom edge.
   */
      ...
      <div style={{ zoom }}>
        <div className="frame" data-tier={tier} data-hinted={touched ? 'off' : 'on'}>
          ...
          <div className="status">
            {/* The status bar is chrome, not content: the page scrolls *under*
                it, so without an opaque backdrop the table and the price sheet
                slide through the time and the icons. This reproduces the top
                45px of the page exactly as the export draws it — the tier
                colour and the same glow, clipped — so the bar looks identical
                at every scroll position instead of picking up whatever happens
                to be passing behind it. */}
...
/** The reassurance line's shield-and-tick, drawn to the export's proportions. */
function ShieldCheck() {
  return (
    <svg viewBox="0 0 14 14" fill="none" aria-hidden>
      <path
        d="M7 1.2 12 3v4.1c0 3-2.1 5-5 5.7-2.9-.7-5-2.7-5-5.7V3l5-1.8Z"
        stroke="currentColor" strokeWidth="1" strokeLinejoin="round"
      />
      <path d="M4.9 7.1 6.3 8.6 9.2 5.5" stroke="currentColor" strokeWidth="1.15"
